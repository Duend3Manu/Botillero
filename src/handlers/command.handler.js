...
async function handleStickerToMedia(client, message) {
    ...
    console.log("(StickerToMedia) -> Iniciando conversión...");
    await message.react('⏳');
    ...
    try {
        console.log("(StickerToMedia) -> Descargando media del sticker...");
        const media = await quotedMsg.downloadMedia();
        if (!media || !media.data) {
            throw new Error("No se pudo descargar la media del sticker.");
        }
        const inputBuffer = Buffer.from(media.data, 'base64');
        console.log(`(StickerToMedia) -> Media descargada. Es animado: ${quotedMsg.isAnimated}`);

        if (quotedMsg.isAnimated) {
            outputPath = path.join(tempDir, `sticker_${Date.now()}.gif`);
            console.log(`(StickerToMedia) -> Convirtiendo a GIF en: ${outputPath}`);
            await sharp(inputBuffer, { animated: true }).gif().toFile(outputPath);
        } else {
            outputPath = path.join(tempDir, `sticker_${Date.now()}.png`);
            console.log(`(StickerToMedia) -> Convirtiendo a PNG en: ${outputPath}`);
        ...
        if (fs.existsSync(outputPath) && fs.statSync(outputPath).size > 0) {
            ...
            await message.react('✅');
            console.log("(StickerToMedia) -> Respuesta enviada con éxito.");
        } else {
            throw new Error('La conversión no generó un archivo de salida válido.');
        }

    } catch (e) {
        console.error("----------- ERROR en handleStickerToMedia -----------");
        console.error(e);
        console.error("----------------------------------------------------");
        await message.react('❌');
        ...
        if (outputPath && fs.existsSync(outputPath)) {
            try {
                ...
                console.log(`(StickerToMedia) -> Archivo temporal eliminado: ${outputPath}`);
            } catch (unlinkErr) {
                console.error(`(StickerToMedia) -> Error al eliminar archivo temporal: ${unlinkErr}`);
            }
        }
    }
}
